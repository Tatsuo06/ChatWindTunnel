...
- **Distributed release over a hemisphere (`source_radius`)**: injecting the full rate into a single ~4 mm surface cell is a point singularity — the effective injection velocity reaches ~10⁴–10⁵ m/s (hypersonic), shocking the compressible solver at emission onset (dt collapse, `bounding k`, force spikes). When `source_radius > 0`, `build_gas_les_restart_case` writes a `system/topoSetDict` (`sphereToCell` at `source_position`, radius `source_radius`) and switches the `fvOptions` source to `selectionMode cellSet; cellSet gasSourceCells`; the gas Allrun runs `runParallel topoSet` (guarded by `[ -f system/topoSetDict ]`) so the rate is spread over the fluid cells within the sphere (a hemisphere in practice — the solid interior has no cells). `source_radius = 0` keeps the single-cell point source (unchanged). The gas-restart forms show a **reference injection velocity** `v ≈ (rate/ρ_gas)/(2πR²)` (ρ_gas = density_ratio × 1.161) and **warn when v ≥ 50 m/s** (or when radius = 0). Rule of thumb: for 0.07 kg/s hydrogen, R ≈ 0.05 m → ~55 m/s.
