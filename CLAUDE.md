...
**refinementBox is computed from the rotated STL bounding box**: The snappyHexMesh refinementBox uses a uniform padding of `0.2 * max(dx, dy, dz)` on all faces except two: the floor gets no padding below (it sits on the ground plane), and the ceiling is lifted to at least half the flow-direction length `0.5*dx` to capture the wake/plume rising above flat/long geometries. The uniform `max`-based padding (rather than per-axis 20%) avoids overly tight boxes for elongated geometries where per-axis 20% would be very small in the narrow directions. (An over-tall ceiling is harmless — `mode inside` only refines cells that exist in the mesh, so any part above the domain is simply ignored.)

```
dx = x1_rot - x0_rot  (and similarly dy, dz)
pad = 0.2 * max(dx, dy, dz)
refbox_min = [x0_rot - pad,  y0_rot - pad,  z0_rot]                    # floor: no padding below (ground)
refbox_max = [x1_rot + pad,  y1_rot + pad,  max(z1_rot + pad, 0.5*dx)]   # ceiling: >= half flow-direction length
```
...
- **refinementBox internal level `levels ((1E15 3))`** — **fixed at 3** in the templates. `_write_snappy_hex_mesh` rewrites only the box's `min`/`max` coordinates, never its level. ⚠️ Because the surface level is user-settable but the box level is not, setting `refinement_max ≤ 3` makes the box interior as fine as / finer than the surface — usually not intended.
...
- **`nCellsBetweenLevels 3`, `resolveFeatureAngle 30`, `maxGlobalCells`** (5M steady/dispersion, 7M legacy LES) — template-fixed, not exposed.

> **Why the box level was 4 but only reached 1, and why `maxGlobalCells` had to rise with it** (learned on the m5480 ship case): snappyHexMesh refines *surface/feature first, then refinementRegions (shells)*. On a fine-surfaced body the surface refinement (level 6) alone consumed the old 2M budget, so when the shell phase started it could lift the box interior only one level (0→1) before hitting `maxGlobalCells` and stopping — the box read as level **1**, not its configured **4**. Crucially `maxGlobalCells` is checked on the **pre-removal** intermediate count (cells later deleted inside the body / outside the keepPoint still count), which is well above the final cell count, so a mesh that finishes at 1.79M can still cap at a 2M limit mid-refinement. Filling the box to level 2 only adds ~40k *final* cells, but needs headroom on the *intermediate* count — hence box level was set to **2** (stops there predictably) **and** `maxGlobalCells` raised to **3M** together. Raising only one of the two would not have worked. **Later raised to box level 3 with `maxGlobalCells` 5M** (same paired-change reasoning: a higher box level needs more intermediate-count headroom to actually reach it).
...
**Reserved (scheduled) restart — run a child after the parent finishes (cluster only)**: a restart child can be created while the parent is still `MESHING`/`RUNNING`/`SCHEDULED` (`restart_job` computes `scheduled = ClusterRunner and parent.job_id and parent not finished`; the per-mode DONE guards are relaxed when `scheduled`). The child's configs are built immediately (they only need the parent's case dir + `case_params.json`, which exist at parent submit), but the **decomposed-solution seed is deferred to the job's run time**: `ClusterRunner.submit(depend_on_job_id, seed_from_in_script)` writes the `cp -al {parent}/processor* .` step into the PBS script (via `_SEED_PREAMBLE_TEMPLATE`) and submits with `qsub -W depend=afterok:<parent_job_id>`. Torque holds the child (`H` state) until the parent completes OK, then releases and runs it — **entirely cluster-side, so the backend/Mac can be down** meanwhile. New status `SimulationStatus.scheduled` (PBS `H` → `SCHEDULED`). If the parent FAILS, Torque drops the held child (its job id reads as "Unknown"); `poll_status` resolves this against the parent (failed parent ⇒ child FAILED, with a transient-qstat-miss guard). Chains work (steady → LES → gas reserved up front) because a held job's id can be depended on. UI: the Run tab shows the restart expanders while the parent runs (gated on `cluster_available`) with a reserve note; scheduled children show ⏳ and can be cancelled (`qdel`). Local runner has no dependency support — it seeds immediately instead (no reservation).
...
- **Distributed release over a sphere (`source_radius`)**: injecting the full rate into a single ~4 mm surface cell is a point singularity — the effective injection velocity reaches ~10⁴–10⁵ m/s (hypersonic), shocking the compressible solver at emission onset (dt collapse, `bounding k`, force spikes). When `source_radius > 0`, `build_gas_les_restart_case` writes a `system/topoSetDict` (`sphereToCell` at `source_position`, radius `source_radius`) and switches the `fvOptions` source to `selectionMode cellSet; cellSet gasSourceCells`; the gas Allrun runs `runParallel topoSet` (guarded by `[ -f system/topoSetDict ]`) so the rate is spread over the fluid cells within `radius` of the point. **Shape = a sphere clipped by the body**: the default auto source (top of geometry + 5%) sits clear of the body, so it is a *full sphere* floating above it; only a source placed on a surface gives a hemisphere. `⚠️ log.topoSet must be removed before runParallel topoSet` (inherited from the parent's meshing → runParallel would skip it → cellSet never built → solver crashes with "cannot find cellSet"). `source_radius = 0` keeps the single-cell point source. The forms show a **conservative reference injection velocity** `v ≈ (rate/ρ_gas)/(2πR²)` (ρ_gas = density_ratio × 1.161; a full sphere exits through 4πR² = half this) and **warn when v ≥ 50 m/s** (or radius = 0). Validated: hydrogen 0.07 kg/s at R = 0.1 → measured near-source |U| ≈ 10 m/s, dt stays ~5e-5 (vs point source: dt collapse 8×, max k 375→26, force std 1.85→0.13).
