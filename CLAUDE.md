...
- **Steady→gas with an aero-LES warm-up (`build_warmup_gas_case` / `_ALLRUN_WARMUP_GAS`)**: seeding the compressible gas solver straight from a steady RANS field makes its `adjustTimeStep` collapse (dt → ~1e-6, never finishes). If `les_warmup_time > 0` on a steady→gas restart, one case runs **Phase A: incompressible pisoFoam LES** (stable from a RANS seed, like the aero LES) for that long, **then Phase B: the normal gas restart** seeded from the warmed-up field. Implementation = `build_gas_les_restart_case` (full gas setup) + warm-up LES configs + a combined Allrun (= LES-restart seed/run + the whole gas-restart body). ⚠️ The warm-up `controlDict.les` must have **empty `functions {}`** — the shared `system/cuttingPlane` samples `( p U T GAS )` for the gas stage, which incompressible pisoFoam can't provide. Warm-up uses fixed dt (`les_delta_t`); the gas stage keeps `adjustTimeStep`. Phases: log.simpleFoam (steady seed) / log.pisoFoam (warm-up) / log.rhoReactingBuoyantFoam (gas) — the existing 3-phase framework handles it. UI: the steady→gas form's "Aero-LES warm-up time [s]" (0 = direct seed).
