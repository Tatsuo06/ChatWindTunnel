...
refbox_max = [x1_rot + pad,  y1_rot + pad,  max(z1_rot + pad, dx)]   # ceiling: >= flow-direction length dx
```
...
- **refinementBox internal level `levels ((1E15 2))`** — **fixed at 2** in the templates. `_write_snappy_hex_mesh` rewrites only the box's `min`/`max` coordinates, never its level. ⚠️ Because the surface level is user-settable but the box level is not, setting `refinement_max ≤ 2` makes the box interior as fine as / finer than the surface — usually not intended.
...
- **`nCellsBetweenLevels 3`, `resolveFeatureAngle 30`, `maxGlobalCells`** (3M steady/dispersion, 7M legacy LES) — template-fixed, not exposed.
