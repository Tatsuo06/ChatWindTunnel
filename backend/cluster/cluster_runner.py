...
class ClusterRunner(JobRunner):
    ...
    def fetch_live_frames(self, case_dir: Path, kind: str = "plane") -> None:
        """Mid-run sync of the animation frame VTKs so the Flow tab can show a
        cutting-plane / streamline animation while the job is still running.

        Heavier than fetch_live_logs (pulls the VTK frame surfaces), so it is
        only called on demand from the visualization endpoints, not on every
        Convergence-tab rerun. Frames are small surface files; --max-size guards
